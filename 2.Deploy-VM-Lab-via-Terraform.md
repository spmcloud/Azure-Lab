...
```
mkdir -p ~/terraform/2.vm-lab/
```

### providers.tf
``` bash
cat << 'EOF' > ~/terraform/2.vm-lab/providers.tf
terraform {
...
### main.tf  
``` bash
cat << 'EOF' > ~/terraform/2.vm-lab/main.tf
...
EOF
```
 - Tags have been added to the VMs, these are not in use currently but will be useful when we start managing the VM's via ansible

### variables.tf
```bash
cat << 'EOF' > ~/terraform/2.vm-lab/variables.tf
variable "home_ip" {
...
### outputs.tf
``` bash
cat << 'EOF' > ~/terraform/2.vm-lab/outputs.tf
...
The below tags have been added to the terraform VM configure. This will allow dynamic inventory management in the Ansible lab.
```
    tags = {
    role = "jump"
  }
