...
EOF
```
- `azurerm_resource_group` - for resource groups
- `azurerm_virtual_network` - for VNets
- `azurerm_subnet` - for subnets
- `azurerm_virtual_machine` - for VMs
- etc.
...
The below tags can been added to the terraform VM configure. This will allow dynamic inventory management in the Ansible lab.
```
    tags = {
    role = "jump"
  }
